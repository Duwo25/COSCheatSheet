...
//  startsWith fails on Android, so replaced with the split and array code
    if (buttonValuePart.length == 5 &&
        buttonValuePart[0] == "page" &&
        buttonValuePart[1] == "timer" &&
        buttonValuePart[2] == "report" &&
...
//      startsWith fails on Android, so replaced with the split and array code
        if (buttonValuePart.length == 3 &&
            buttonValuePart[0] == "page" &&
            buttonValuePart[1] == "timer") { 
            rowid = buttonValuePart[2];
            localStorage.setItem("currentTMAgendaRow",Number(rowid));
            $(':mobile-pagecontainer').pagecontainer("change", "#page-timer", {transition: "slide"});
        } else {
            switch (buttonValue) {
...
$(document).on('pagebeforeshow','#page-import-agenda', function(event){ 
    $('#agendaContent').val("");
});
...
/*
    if (navigator.userAgent.match(/(Android)/)) {
        navigator.app.loadUrl('http://tmclub.eu/view_meeting.php?t=next&pr=2', {openExternal:true});
    } else {
        window.open('http://tmclub.eu/view_meeting.php?t=next&pr=2','_system','location=yes');
    }
*/
    if (isRunningAsApp()) {
        cordova.InAppBrowser.open('http://tmclub.eu/view_meeting.php?t=next&pr=2', '_blank', 'location=yes');
    } else {
        window.open('http://tmclub.eu/view_meeting.php?t=next&pr=2','_system','location=yes');
    }
}
...
function importAgenda() {
    ...
    if (agendaDetails == null || agendaDetails == "") {
        ...
        $("#popup-import-without-data #popupImportWithoutDataButtonContinue" ).on( "click", function() {
            parseTextToAgenda();
            setTimeout(function(){
                $('#NumAgendaLinesImported').html("Agenda is now empty.");
            ...
            /*
            There seems to be an issue with the code below that causes the navigation to go back an extra page each time you use it.
        ...
        $("#popup-import-without-data #popupImportWithoutDataButtonContinue" ).on( "popupafterclose", function() {
            $( "#popup-import-without-data #popupImportWithoutDataButtonContinue" ).unbind( "click"); 
        });
    } else {
        parseTextToAgenda();
        setTimeout(function(){
            var arrTMAgenda = JSON.parse(localStorage.getItem("TMAgenda"));
            $('#NumAgendaLinesImported').html(JSON.parse(localStorage.getItem("TMAgenda")).TMAgenda.length + " item(s) have been saved.");
            $('#popup-import-agenda-complete').popup();
            $('#popup-import-agenda-complete').popup('open');
            }, 100);
        setTimeout(function(){
...
function parseTextToAgenda() {
    ...
    var arrTMAgenda = {"TMAgenda":[]};
    var objAgendaItem = {};
    var agendaRecord = {};


    var arrAgendaDetailLine = agendaDetails.split(/\r\n|\r|\n/g); // breaks the file in array of lines
        ...
        switch (arrAgendaFields.length) {
            ...
            case 3:
                ...
                if (agendaItem == true) {
                    ...
                    // Write AgendaItem
                    //console.log(agendaTime + "|" + agendaRole + "|" + agendaPresenter + "|" + agendaEvent + "|" + agendaDurationGreen + "|" + agendaDurationAmber + "|" + agendaDurationRed);
            ...
            case 5:
                ...
                break;
            default:
                console.log ("Unhandled number of fields on line:" + arrAgendaFields.length);
        }
    }
    ...
    if (arrTMAgenda.TMAgenda.length == 0) {
        ...
        var fieldNum = 0;
        var numFields = 0;
        var arrField = ["mTime","Role","Presenter","Manual","Project","Event","Description","DurationGreen","DurationAmber","DurationRed"];
        
        agendaSection = false;
        for (var i=0;i < arrAgendaDetailLine.length; i++) {
            // as long as we haven't reached that line, we can continue searching for it
            if (agendaSection == false && arrAgendaDetailLine[i] != "Agenda") { continue; }
            // if the line only contains "Agenda", we've reached the Agenda section and the real work begins
            if (arrAgendaDetailLine[i] == "Agenda") {
                agendaSection = true;
                agendaRecord = initializeAgendaRecord();
                continue;
            }
            // when we find line with "Attendance", then we've reached the end of the agenda
            ...
            if (arrFields.length == 1 || arrFields.length == 3) {
                ...
                for (var w=0; w<arrFields.length; w++) {
                    ...
                    if (arrFields[w].length >= 4 && arrFields[w].length <= 5) {
                        // if the length of the word equals 5 then try to split it in MM and SS 
                        var arrPortion = arrFields[w].split(":");
                        if (arrPortion.length    == 2 &&
                            arrPortion[0].length >= 1 && 
                            arrPortion[0].length <= 2 && 
                            arrPortion[1].length == 2) {
                            // More detailed checks could be done, like checking for numeric values
            ...
            switch (numTimeValues) {
                case 1:
                    ...
                    break;
                case 3:
                    //console.log("Green: " + arrFields[0] + ", Amber: " + arrFields[1] + ", Red: " + arrFields[2]);
                ...
                case 0:
                    ...
                    switch (arrField[numFields - 1]) {
                        ...
                        case "Event":
                            ...
                            //console.log(arrField[numFields - 1] + ": " + arrAgendaDetailLine[i]);
                            break;
                    }
            }
...
function addRecordToAgenda(arrTMAgenda, agendaRecord) {
    ...
    if (agendaRecord["mTime"]         == null) { agendaRecord["mTime"]         = ""; }
    if (agendaRecord["Role"]          == null) { agendaRecord["Role"]          = ""; }
    if (agendaRecord["Presenter"]     == null) { agendaRecord["Presenter"]     = ""; }
    if (agendaRecord["Event"]         == null) { agendaRecord["Event"]         = ""; }
    if (agendaRecord["DurationGreen"] == null) { agendaRecord["DurationGreen"] = ""; }
    if (agendaRecord["DurationAmber"] == null) { agendaRecord["DurationAmber"] = ""; }
    if (agendaRecord["DurationRed"]   == null) { agendaRecord["DurationRed"]   = ""; }
...
$(document).on("pageshow","#page-agenda", function(event) {
    ...
    if (stayAwake == true) {
        if (isRunningAsApp()) {window.plugins.insomnia.keepAwake();}
    } else {
        if (isRunningAsApp()) {window.plugins.insomnia.allowSleepAgain();}
    }
...
function checkMeetingTimer () {
    ...
    $('.check-time').each(function() {
        ...
        if (currentHours > topicHours) {
            pastTime = true;
        } else if (currentHours == topicHours && currentMinutes >= topicMinutes) {
            pastTime = true;
        }
        if (pastTime) { $(this).removeClass("check-time").addClass("past-time");}
    });
...
/*
                        ...
                        <thead>
                            <tr>
                                ...
                                <th>Timer</th>
                                <th>Role</th>
                                <th data-priority="2">Presenter</th>
                                <th data-priority="1">Event</th>
                                <th data-priority="6">Green</th>
                                <th data-priority="6">Amber</th>
                                <th data-priority="6">Red</th>
                            </tr>
                        </thead>
                        <tbody>
                            <!-- Table content is dynamically added here -->
                        </tbody>

*/    
    $('#AgendaTable').html('<thead><tr><th>Time</th><th>Timer</th><th>Role</th><th data-priority="2">Presenter</th><th data-priority="1">Event</th><th data-priority="6">Green</th><th data-priority="6">Amber</th><th data-priority="6">Red</th></tr></thead><tbody></tbody>');
    ...
    if (arrAgenda == null) {
        ...
        for(var i = 0; i < arrAgenda.TMAgenda.length; i++) {
            ...
            if (buttonTimer == true) {
                $('#AgendaTable tbody').append('<tr id="'+ agendaItemId + '"></tr>');
            } else {
                $('#AgendaTable tbody').append('<tr id="'+ agendaItemId + '" onclick="buttonClick(&quot;page-timer-' + i + '&quot;)"></tr>');
            }
            $('#' + agendaItemId).append('<td data-theme="a"><div class="check-time">' + arrAgenda.TMAgenda[i].mTime + '</div></td>');
            $('#' + agendaItemId).append('<td><a href="#" class="row-button ui-btn ui-mini ui-corner-all ui-btn-icon-notext ui-icon-clock ui-nodisc-icon" onclick="buttonClick(&quot;page-timer-' + i + '&quot;)">Timer</a></td>');
            $('#' + agendaItemId).append('<td>' + arrAgenda.TMAgenda[i].Role + '</td>');
            $('#' + agendaItemId).append('<td>' + arrAgenda.TMAgenda[i].Presenter + '</td>');
            $('#' + agendaItemId).append('<td>' + arrAgenda.TMAgenda[i].Event + '</td>');
            $('#' + agendaItemId).append('<td>' + arrAgenda.TMAgenda[i].DurationGreen + '</td>');
            $('#' + agendaItemId).append('<td>' + arrAgenda.TMAgenda[i].DurationAmber + '</td>');
            $('#' + agendaItemId).append('<td>' + arrAgenda.TMAgenda[i].DurationRed + '</td>');
        }
        ...
        var columnIndex = 0;
        $("#AgendaTable fieldset").find("input").each(function(){
             var sel = ":nth-child(" + (columnIndex + 1) + ")";
             $(this).jqmData("cells", $("#AgendaTable").find("tr").children(sel));
             columnIndex++;
        });
...
$(document).on('pagebeforeshow','#page-timer', function(event){ 
    ...
    /* In case timer is enabled and settings have been changed, make the necessary changes */
    if (timerEnabled) {
        objSettings = JSON.parse(localStorage.getItem("TMSettings"));
        if (objSettings != null) {
...
function setTimerData() {
    ...
    if (rowid != null) {
        ...
        if (arrAgenda != null && arrAgenda.TMAgenda.length >= rowid) {
            ...
            $('#timerComment').val(rowAgenda.Role + " - " + rowAgenda.Presenter + " - " + rowAgenda.Event);
            if (rowAgenda.DurationGreen != null && rowAgenda.DurationGreen != "") {
                arrTime = rowAgenda.DurationGreen.split(":");
                $('#timerGreenMinutes').val(arrTime[0]).selectmenu("refresh");
                $('#timerGreenSeconds').val(arrTime[1]).selectmenu("refresh");
            } else {
                $('#timerGreenMinutes').val("0").selectmenu("refresh");
                $('#timerGreenSeconds').val("00").selectmenu("refresh");
            }
            if (rowAgenda.DurationAmber != null && rowAgenda.DurationAmber != "") {
                arrTime = rowAgenda.DurationAmber.split(":");
                $('#timerAmberMinutes').val(arrTime[0]).selectmenu("refresh");
                $('#timerAmberSeconds').val(arrTime[1]).selectmenu("refresh");
            } else {
                $('#timerAmberMinutes').val("0").selectmenu("refresh");
                $('#timerAmberSeconds').val("00").selectmenu("refresh");
            }
            if (rowAgenda.DurationRed != null && rowAgenda.DurationRed != "") {
                arrTime = rowAgenda.DurationRed.split(":");
                $('#timerRedMinutes').val(arrTime[0]).selectmenu("refresh");
                $('#timerRedSeconds').val(arrTime[1]).selectmenu("refresh");
            } else {
                $('#timerRedMinutes').val("0").selectmenu("refresh");
                $('#timerRedSeconds').val("00").selectmenu("refresh");
            }
        }
        resetTimer();
...
function updateTimer() {
    ...
    newTimerValue (Math.round((timerTotalMilliseconds + timerMilliseconds) / 1000));
}
...
function startStopTimer() {
    ...
    if (timerEnabled == true) {
        ...
        $('#page-timer a[role="button"][data-rel="back"]').removeClass("ui-state-disabled"); // Back button
        ...
        $('#page-timer a[role="button"][data-rel="back"]').addClass("ui-state-disabled"); // Back button
        ...
        $('#timerGreenMinutes').selectmenu('disable');
        $('#timerGreenSeconds').selectmenu('disable');
        $('#timerAmberMinutes').selectmenu('disable');
        $('#timerAmberSeconds').selectmenu('disable');
        $('#timerRedMinutes').selectmenu('disable');
        $('#timerRedSeconds').selectmenu('disable');
        newTimerValue(Math.round(timerTotalMilliseconds / 1000));
        // Calculate color times
        var $greenM = $('#timerGreenMinutes');
        var $greenS = $('#timerGreenSeconds');
        var $amberM = $('#timerAmberMinutes');
        var $amberS = $('#timerAmberSeconds');
        var $redM = $('#timerRedMinutes');
        var $redS = $('#timerRedSeconds');

        timerGreen = Number($greenM.val()) * 60 + Number($greenS.val());
        timerAmber = Number($amberM.val()) * 60 + Number($amberS.val());
        timerRed   = Number($redM.val())   * 60 + Number($redS.val()); 
        objSettings = JSON.parse(localStorage.getItem("TMSettings"));
        if (objSettings != null) {
            stayAwake = objSettings.stayAwakeWhileTiming;
        }
...
function resetTimer() {
    ...
    $('#timerGreenMinutes').selectmenu('enable');
    $('#timerGreenSeconds').selectmenu('enable');
    $('#timerAmberMinutes').selectmenu('enable');
    $('#timerAmberSeconds').selectmenu('enable');
    $('#timerRedMinutes').selectmenu('enable');
    $('#timerRedSeconds').selectmenu('enable');
...
function setNewColor(newColor) {
    ...
    if (fileColor != "") { 
        srcFull=fileFolder + prefix + fileColor + "_" + imageSize + "x" + imageSize + suffix;
    } else {
...
function saveTimerData() {
    ...
    if (colorToSave == "grey") {colorToSave="";}
    saveDataToLocalStorage (String(timerId), $('#timerComment').val(), $('#timer').text().substr(3), colorToSave);
...
function saveDataToLocalStorage (timerId,timerComment,timerDuration,timerLight){
...
function updateTimerSettingsTitle (timerColor) {
    ...
    //console.log(timerColor + " " + $('#timer' + timerColor + 'Minutes').val() + ":" + $('#timer' + timerColor + 'Seconds').val());
    var minutes = $('#timer' + timerColor + 'Minutes').val();
    var seconds = $('#timer' + timerColor + 'Seconds').val();
    
    if (minutes == 0 && seconds == 0) {
        $('#Setting' + timerColor).html("");
    } else {
        $('#Setting' + timerColor).html("&nbsp;" + minutes + ":" + seconds + "&nbsp;");
    }
...
$(document).on('pagebeforeshow','#page-timer-report', function(event){ 
    ...
    $('#page-timer-report #report-summary').collapsible( "option", "collapsed", false );
...
function readTimerDataFromLocalStorage (){
    ...
    if (timerArr == null) { 
        $('#NumEarly').prop("innerHTML", totalEarly + "&nbsp;(" + pctEarly + "%)");
        $('#NumGreen').prop("innerHTML", totalGreen + "&nbsp;(" + pctGreen + "%)");
        $('#NumAmber').prop("innerHTML", totalAmber + "&nbsp;(" + pctAmber + "%)");
        $('#NumRed').prop("innerHTML", totalRed + "&nbsp;(" + pctRed + "%)");
        $('#page-timer-report #summary-chart').hide(); 
        return; 
    } else {
        $("#ListDiv").append('<ul id="List" data-role="listview" data-filter="true" data-input="#timerReportFilter" data-inset="true"></ul>');
        for(i = 0; i < timerArr.TMTimed.length; i++) {
            ...
            if (timerLight == "black") { timerLight = ""; }
            switch (timerLight) {
                case ""      : {
                    totalEarly++;
                    if ($("#earlyTime").prop("checked")) { createRow = true; };
                    break;
                }
                case "green" : {
                    totalGreen++;
                    if ($("#greenTime").prop("checked")) { createRow = true; };
                    break;
                }
                case "amber" : {
                    totalAmber++;
                    if ($("#amberTime").prop("checked")) { createRow = true; };
                    break;
                }
                case "red"   : {
                    totalRed++;
                    if ($("#redTime").prop("checked")) { createRow = true; };
                    break;
                }
            }
            if (createRow) {
                if (timerLight == "") {timerImage = "blank";} else {timerImage = timerLight;}
                timerImage = "images/mark_" + timerImage + "_300x300.png"
//              $('#List').append("<li><table><tr><td><span class='timercolor-" + timerLight + " + timer-li'>" + timerDuration + "</span></td><td width='60px'>(" + timerLight + ")</td><td>" + timerComment + "</td></tr></table></li>");
                htmlRow = "<li id='timerReportRow-" + i + "'>"
                        +   "<a href='#'><table>"
                        +     "<tr>"
                        +       "<td>" + timerDuration + "</td>"
                        +       "<td><img src='" + timerImage + "' width='16px'></td>"
                        +       "<td>" + timerComment + "</td>"
                        +     "</tr>"
                        +   "</table></a>"
                        +   "<a href='#' class='ui-btn ui-mini ui-btn-icon-notext ui-icon-delete' onclick='buttonClick(&quot;page-timer-report-deleteRow-" + i + "&quot;)'>Delete</a>"
                        + "</li>";
                $('#List').append(htmlRow);
            }
        }
        $('#List').listview();
        /*
        pctEarly = Math.round(totalEarly / timerArr.TMTimed.length * 100 * 100) / 100;
        pctGreen = Math.round(totalGreen / timerArr.TMTimed.length * 100 * 100) / 100;
        pctAmber = Math.round(totalAmber / timerArr.TMTimed.length * 100 * 100) / 100;
        pctRed   = Math.round(totalRed   / timerArr.TMTimed.length * 100 * 100) / 100;
        */
...
function deleteTimerReportRow(rowid) {
    ...
    $("#popup-timer-report-confirm-delete #popupTimerReportConfirmDeleteButtonContinue").on("click", function() {
//        console.log("deleting row with rowid " + rowid);
    ...
    $("#popup-timer-report-confirm-delete").on("popupafterclose", function() {
        $("#popup-timer-report-confirm-delete #popupTimerReportConfirmDeleteButtonContinue").unbind("click");
    });
...
function removeDataFromLocalStorage () {
    ...
    $("#popup-timer-report-confirm-delete-all #popupTimerReportConfirmDeleteButtonContinue").on("click", function() {
        localStorage.removeItem("TMTimed");
        readTimerDataFromLocalStorage();
//        $("#popup-timer-report-confirm-delete-all #popupTimerReportConfirmDeleteButtonContinue").unbind("click");
    });
    $("#popup-timer-report-confirm-delete-all").on("popupafterclose", function() {
        $("#popup-timer-report-confirm-delete-all #popupTimerReportConfirmDeleteButtonContinue").unbind("click");
    });
...
function readSettingsFromLocalStorage() {
    ...
    if (objSettings != null) {
        ...
        if (agendaTimerButton || agendaTimerButton == null) {
            $('#timerbutton-agenda-setting').prop("checked","true").checkboxradio("refresh");
            $('#row-selection-agenda-setting').checkboxradio("refresh");
        } else {
            $('#row-selection-agenda-setting').prop("checked","true").checkboxradio("refresh");
...
function isRunningAsApp() {
    // if protocol used is http or https then the app runs in a browser. If it's a Cordova app, it uses the file:// protocol.
